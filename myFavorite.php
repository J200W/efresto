...
$sql = "SELECT restaurant_id FROM favorite where user_id = '$uid'";
...
<body>
    ...
    <h1 class="titlePage">My Favorites</h1>
    <hr>

    <div class="result-box">
        <?php
        while ($rid = $result->fetch_row()) {
            $sql2 = "SELECT * FROM restaurant where restaurant_id = '$rid[0]'";
            $result2 = $conn->query($sql2);
            while ($row2 = $result2->fetch_row()) {
                ?>
                <a href="restaurant.php?id=<?php echo $row2[0]; ?>">
                    <div class="result">
                        <div class="image-space">
                            <?php echo '<img class="result-photo" src="data:image;base64,' . get_photo($rid[0]) . '" alt="' . $row2[1] . ' photo">'; ?>
                        </div>
